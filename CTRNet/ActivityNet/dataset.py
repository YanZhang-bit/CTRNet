...
class VideoDataSet(data.Dataset):  # zuikaishidechengxu
...
class VideoDataSet_change(data.Dataset):
    def __init__(self, opt, subset="train"):
        self.temporal_scale = opt["temporal_scale"]  # 100
        self.temporal_gap = 1. / self.temporal_scale
        self.subset = subset
    ...
    def _get_train_label(self, index, anchor_xmin, anchor_xmax):
        ...
        for i in range(self.temporal_scale):
            a = i // 2
            for j in range(self.temporal_scale - a):
                if i + j + 1 < self.temporal_scale:
                    gt_iou_map[i, j + a] = np.max(
                        iou_with_anchors(j * self.temporal_gap, (j + i + 1) * self.temporal_gap, gt_xmins, gt_xmaxs))
        gt_iou_map = torch.Tensor(gt_iou_map)
...
class VideoDataSet_change_tsp(data.Dataset):
    def __init__(self, opt, subset="train"):
        self.temporal_scale = opt["temporal_scale"]  # 100
        self.temporal_gap = 1. / self.temporal_scale
        self.subset = subset
    ...
    def __getitem__(self, index):
        video_data = self._load_file(index)
        if self.mode == "train":
            match_score_start, match_score_action,match_score_end, confidence_score = self._get_train_label(index, self.anchor_xmin,
                                                                                         self.anchor_xmax)
            return video_data, [[confidence_score,match_score_action, match_score_start, match_score_end]]
        else:
            return index, video_data

    def _load_file(self, index):
        video_name = self.video_list[index]
        video_df = np.load(self.feature_path + "BMN_feature/" + video_name + ".npy")
        video_data = torch.from_numpy(video_df)
        video_data = torch.transpose(video_data, 0, 1)
        video_data.float()
        return video_data
    ...
    def _get_train_label(self, index, anchor_xmin, anchor_xmax):
        ...
        for jdx in range(len(anchor_xmin)):
            match_score_action.append(np.max(
                ioa_with_anchors(anchor_xmin[jdx], anchor_xmax[jdx], gt_xmins, gt_xmaxs)))
        match_score_start = torch.Tensor(match_score_start)
        match_score_end = torch.Tensor(match_score_end)
        match_score_action = torch.Tensor(match_score_action)
        ############################################################################################################

        return match_score_start,match_score_action, match_score_end, gt_iou_map
