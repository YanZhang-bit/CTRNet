...
class Attention_Conv_Fusion_module(nn.Module):
    def __init__(self, in_channels, out_channel1, out_channel2, kernel_size, stride=1,padding=1,dilate=1, bias=False):
        ...
        self.conv2 = PacConv2d(in_channels=in_channels, out_channels=out_channel2, kernel_size=kernel_size,
                                 stride=stride,padding=padding,dilation=dilate,bias=bias)
...
if __name__ == '__main__':
    ...
    opt = opts.parse_opt()
    opt = vars(opt)
    model=Content_Relation_Module_adpac(20,30,40,3,1,1).cuda()
