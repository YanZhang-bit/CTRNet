...
def Soft_NMS(df,tiou=0, nms_threshold=1e-5, num_prop=200):
    '''
    From BSN code
    :param df:
    :param nms_threshold:
...
def _gen_proposal_video(video_name,opt, num_prop=200):
    files = [opt['output'] + "/results/" + f for f in os.listdir(opt['output'] + "/results/") if
             video_name in f]
    if len(files) == 0:
        # raise FileNotFoundError('Missing result for video {}'.format(video_name))
    ...
    for snippet_file in files:
        snippet_df = pd.read_csv(snippet_file)
        start_sorce=(snippet_df.xmin_score.values[:]>opt['start_thr']).astype('float32')
        end_sorce = (snippet_df.xmax_score.values[:] > opt['end_thr']).astype('float32')
        mid_sorce=(snippet_df.mid_scorce.values[:] > opt['action_thr']).astype('float32')
        snippet_df['score'] = snippet_df.clr_score.values[:] * snippet_df.reg_socre.values[:]*end_sorce*start_sorce*mid_sorce
        #snippet_df = Soft_NMS(snippet_df, nms_threshold=opt['nms_thr'])
        dfs.append(snippet_df)
    df = pd.concat(dfs)
    if len(df) > 1:
        df = Soft_NMS(df,tiou=opt["tiou_thr"], nms_threshold=opt['nms_thr'])
    df = df.sort_values(by="score", ascending=False)
...
def gen_proposal_multicore(opt):
    # get video list
    thumos_test_anno = pd.read_csv("./data/thumos_annotations/test_Annotation.csv")
    video_list = thumos_test_anno.video.unique()
    thumos_gt = pd.read_csv("./data/thumos_annotations/thumos14_test_groundtruth.csv")
    ...
    # load video level classification
    parallel = Parallel(n_jobs=15, prefer="processes")
    detection = parallel(delayed(_gen_proposal_video)(video_name,opt)
